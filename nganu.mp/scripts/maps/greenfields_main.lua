...
function M.OnGameModeInit()
    print("Map script loaded: greenfields_main")
end
...
function M.OnPlayerEnterMap(playerid)
    ...
    local biome = GetPlayerMapProperty(playerid, "biome") or "the world"
    local map_id = GetPlayerMapId(playerid)
    SendPlayerMessage(playerid, string.format("Welcome to %s, %s!", biome, name))
    SendPlayerMessage(playerid, map_objects.map_population_line(playerid))
    BroadcastMapMessage(map_id, string.format("%s entered %s.", name, map_id))
    starter_quest.sync_objective(playerid)
end

function M.OnPlayerLeaveMap(playerid)
    BroadcastMapMessage(GetPlayerMapId(playerid), string.format("%s left %s.", GetPlayerName(playerid), GetPlayerMapId(playerid)))
end
...
function M.OnPlayerText(playerid)
    print(string.format("[chat] %s: %s", GetPlayerName(playerid), GetLastPlayerText()))
end
...
function M.OnMapObjectInteract(playerid, object_index)
    ...
    if kind == "prop" then
        map_objects.describe_prop(playerid, object_index, "An old roadside marker stands here.")
    end
end
