...
- `nganu.atlas/`: `raylib` atlas and map editor.
...
- `./build/nganu-atlas`: run the atlas editor.
...
The server requires LuaJIT development files. The client and atlas editor require `raylib`.
...
- Manual runs of `nganu.game` and `nganu.atlas` for gameplay/editor changes.
