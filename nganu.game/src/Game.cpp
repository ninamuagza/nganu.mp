...
bool AndroidPointPressedInCircle(Rectangle rect, bool& wasDown) {
...
bool AndroidInteractPressed() {
    ...
    return AndroidPointPressedInCircle(AndroidActionButtonRect(0.0f), wasDown);
}

bool AndroidInventoryPressed() {
    static bool wasDown = false;
    return AndroidPointPressedInCircle(AndroidActionButtonRect(1.0f), wasDown);
}

bool AndroidChatPressed() {
    static bool wasDown = false;
    return AndroidPointPressedInCircle(AndroidActionButtonRect(3.0f), wasDown);
}

bool AndroidObjectivePressed() {
    static bool wasDown = false;
    return AndroidPointPressedInCircle(AndroidActionButtonRect(2.0f), wasDown);
}
...
void Game::StartLogin() {
    ...
    if (!mapReady_) {
        loginStatus_ = "Refreshing map content...";
        BeginBootUpdateCheck();
        return;
    }
    if (!network_.IsConnected() || !handshakeReady_) {
        loginStatus_ = "Server session changed. Reconnecting...";
        BeginBootUpdateCheck();
        return;
    }
...
void Game::DrawLoginScreen() const {
    ...
    std::string buttonText = "Check Server";
    if (uiMode_ == UiMode::LoggingIn) {
        buttonText = "Spawning...";
    } else if (canEnterWorld) {
        buttonText = "Enter World";
    } else if (manifest_.valid && !mapReady_) {
        buttonText = "Load Content";
    } else if (manifest_.valid && mapReady_ && !network_.IsConnected()) {
        buttonText = "Reconnect";
    } else if (manifest_.valid && mapReady_ && network_.IsConnected() && !handshakeReady_) {
        buttonText = "Sync Session";
    }
